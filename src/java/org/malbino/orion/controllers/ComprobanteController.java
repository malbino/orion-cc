 ...
 * @author Tincho
 */
@Named("ComprobanteController")
@SessionScoped
public class ComprobanteController extends AbstractController implements Serializable {

    @EJB
    ComprobanteFacade comprobanteFacade;
    @EJB
    CajasFacade pagosFacade;
    @Inject
    ...
    public void anularComprobante() {
        if (pagosFacade.anularComprobante(seleccionComprobante)) {
            //log
            logFacade.create(new Log(Fecha.getDate(), EventoLog.UPDATE, EntidadLog.COMPROBANTE, seleccionComprobante.getId_comprobante(), "Anulación de comprobante", loginController.getUsr().toString()));

            reinit();
        } else {
            this.mensajeDeError("No se pudo anular el comprobante.");
        }
    }

    public void toComprobantes() throws IOException {
        this.redireccionarViewId("/cajas/comprobantes/comprobantes");
    }

    public void toComprobantePago() throws IOException {
        this.redireccionarViewId("/cajas/comprobantes/comprobantePago");
    }
